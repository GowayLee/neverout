...
public class LevelController {
    ...
    public LevelController (Scene scene) {
        ...
        this.mainMenu = new MainMenu(scene, mainMenuListener); // 初始化主菜单，传入关卡配置信息，用于显示关卡选择
        this.pauseMenu = new PauseMenu(scene, pauseMenuListener);
        this.levelMenu = new LevelMenu(scene);
        this.gameOverMenu = new GameOverMenu(scene); // 初始化游戏结束菜单，传入关卡配置信息，用于显示关卡选择
    }
    ...
    public void startLevel() { // 初始化引擎层,开始关卡逻辑
        gameEngine = new GameEngine(config, scene, engineLayerListener);
